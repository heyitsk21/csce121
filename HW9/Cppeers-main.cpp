...
void processPostsByUser(Network& cppeers) {
  ...
  cout << "Enter username: ";
  cin >> userName;
  //cout<< cppeers.getPostsByUser(userName) << endl;
  //cout << "Posts by useName {" << userName << "}:" << endl;
  vector<Post*> temp = cppeers.getPostsByUser(userName);
  string outputStr = "";
  for (Post* ptemp : temp){
    outputStr = ptemp->getPostText();
    cout << outputStr.substr(1,outputStr.size()) << endl;
  }
}
...
void processPostsWithHashtags(Network& cppeers) {
  ...
  cout << "Enter tagname: ";
  cin >> hashtag;
  vector<Post*> temp = cppeers.getPostsWithTag(hashtag);
  string outputStr = "";
  for (Post* ttemp : temp){
    outputStr = ttemp->getPostText();
    cout << outputStr.substr(1,outputStr.size()) << endl;
  }
}
...
int main() {
  ...
  try {
    ...
    } while (choice != 9 && choice>0 && choice<5);
